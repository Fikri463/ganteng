...
*NOMOR CREATOR*
▼▼▼▼▼`)
sendKontak(from, `6287828930441`, `${ownerName}`, 'Sibukk!!')
               await sleep(1000)
               break
               case 'spamcall':{
...
case 'lock':
 ikymemek = {
  "key": {
    "fromMe": false,
    participant: `6287828930441@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
  },
  "message": {
    "viewOnceMessage": {
      "message": {
...
🐟 ${prefix}wallgravity text1 text2
 `
sendButMessage(from, wibu, `created wanz bot☕`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
`
        ...
        case 'wasted':
		      ...
		      buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Jangan Lupa Follow @wanz_store_05`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'wanz bot☕', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @wanz_store_05`,buttons,headerType:4}
...
case 'chiisaihentai':
              ...
              ikyy.relayWAMessage(prep)
               ...
               buff = await getBuffer(`https://api.lolhuman.xyz/api/random2/${command}?apikey=${setting.lolkey}`)
                buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'wanz botz☕', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @wanz_store_05`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
              ...
              ikyy.relayWAMessage(prep)
                ...
                reply (mess.wait)
                  ...
                  buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${setting.lolkey}&text=${ini_txt}`)
                          buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'wanz botz☕T', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @wanz_store_05`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
...
`
 ...
 wanz botz☕︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     case 't':
       titid =  `SILAHKAN PILIH DI BAWAH UNTUK MELANJUTKAN`
                   ...
                   sendButMessage(from, titid, `crated wanz botz☕︎`, [
          {
            buttonId: `Y`,
            buttonText: {
              displayText: `Y`,
...
*「 wanz botz☕︎ 」*`
	    ...
	    // console.log(e)
            setTimeout( () => {
			ikyy.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Mendaftar Di Wanz Bot🐟*`)
		}, 2000)
        break
case 'sfire':
{
...
`
  buttons = [{buttonId: `${prefix}buypremium`,buttonText:{displayText: `Premium`},type:1},{buttonId: `${prefix}payment`,buttonText:{displayText: `PAYMENT`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'wanz botz☕︎ ', imageMessage: imageMsg,
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport': 
              ...
              ikyy.sendMessage('6287828930441@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'jawaban':
       ...
       case 'request':
              ...
              ikyy.sendMessage('6287828930441@s.whatsapp.net',`*Request:* ${teks}`, text)
              break
case 'chatrava':
              if (args.length < 1) return reply(`Ketik ${prefix}CHAT [CHAT apa]`) 
              teks = args.join(' ')
              reply('CHAT RAVA DONE')
              ikyy.sendMessage('6287828930441@s.whatsapp.net',`*chat:* ${teks}`, text)
              break
       case 'readall':
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Jangan Lupa Follow Ig Owner Ya : https://instagram.com/wanz_store_05?utm_medium=copy_link`)
             break
      case 'hamdeh':
             reply('Hamdeh hamdeh')
             break
